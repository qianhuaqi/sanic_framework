# LingShu Framework 总体架构设计总纲（P0-RC2）
...
> `P0_DECISION_STATUS.md` 中标记为 Confirmed 的内容，才可以成为实施依据。
> Proposed 与 Candidate 内容均不得创建生产源码、包骨架或运行时依赖。
...
下层机制不得反向依赖业务能力、具体数据库、认证实现、项目代码或测试工具。
...
框架核心、官方能力、测试、文档、示例、工具、安全测试和发布元数据原则上在
该仓库统一治理。除非未来 ADR 证明必须拆分，否则不另建组件仓库。
...
## 7. P0-D3 打包与组件布局提案（Proposed）

项目负责人已明确：**不采用 `src/` 目录。**

本节必须在 P0-D3 PR 合并后才成为 Confirmed。
...
- 所有内部组件共享一个版本和发布节奏；
- 前期不发布 `lingshu-core`、`lingshu-http`、`lingshu-server` 等独立包；
- 未来拆包必须通过新的 ADR 和真实独立消费者证据。
...
扩展注册、依赖排序、Capability 绑定、生命周期、健康和清理机制。不得因为某项
能力未来可能是扩展，就把 Auth、Tenant、SQL、Redis 等具体政策放入本模块。
...
禁止依赖环、下层导入根 facade、生产代码导入 testing、以及跨组件导入私有模块。
...
- 深层路径默认属于内部实现；
- `_` 开头的模块或名称是私有的；
- import 时禁止启动任务、建立连接或修改进程级状态；
- 具体公共类名和函数名不在 P0-D3 冻结。

### 7.6 可选依赖

单 distribution 不代表所有集成都成为必装依赖。

- `import lingshu` 不得要求数据库、Redis、云平台、Tracing 或认证库；
- 可选能力只在激活时加载；
- 缺失依赖只在相关能力激活时给出明确错误；
- 可选依赖不得成为隐藏 Core 依赖；
- 具体 extras 和官方扩展目录后续决定。

### 7.7 打包质量门

因为源码直接位于根级 `lingshu/`，仓库内测试通过不能证明安装包正确。
...
- Python 最低版本；
- 构建后端；
- 具体公共 Application、Request、Response、Scope、Deadline、Limiter 与 Cancellation 名称；
- 公共 API manifest 形式；
- 权威版本来源文件；
- optional extras 和官方扩展目录；
- 首个 PyPI 版本发布时间；
- v1.0 后兼容政策。
...
## 10. P0 Hardening 合并要求

P0-D2 已确认 monotonic Deadline、Async Context 隔离、运行时资源有界和部分
Telemetry 要求。
...
`P0_HARDENING_CHECKLIST.md` 最终必须归档或改为验收记录，不能成为第二事实源。
