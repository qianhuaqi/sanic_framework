...
def params_exists(request: Request, params, name, msg=None):
    if name not in params or (params[name] != 0 and not params[name]):
        ...
        msg = msg if msg else get_error_message(request, code)
...
def over_limit(request: Request, param_name, param, min_length=-1, max_length=100):
    ...
    if max_length == -1:
        code = 991108
        msg = get_error_message(request, code).format(param_name, min_length)
        raise APIException(code=code, msg=msg, status_code=400)
    elif min_length == -1:
        code = 991107
        msg = get_error_message(request, code).format(param_name, max_length)
        raise APIException(code=code, msg=msg, status_code=400)
        ...
        code = 991105
        msg = get_error_message(request, code).format(param_name, min_length, max_length)
        raise APIException(code=code, msg=msg, status_code=400)
