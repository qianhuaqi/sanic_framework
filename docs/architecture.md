...
`sanic-framework init my_api` creates only the public project skeleton. It does not create `app/v1`. Additional versions can be scaffolded with `sanic-framework add v1`.

`app/v1/controller`, `app/v1/model/table`, `app/v1/model/business`, and `app/v1/view` are the default versioned business development locations. RESTful modules can be created with `sanic-framework make module v1 demo`.
