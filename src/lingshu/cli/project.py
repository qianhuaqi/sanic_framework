...
def _imports_legacy_framework(module: ast.Module) -> bool:
    for node in ast.walk(module):
        if isinstance(node, ast.Import):
            if any(alias.name == "framework" or alias.name.startswith("framework.") for alias in node.names):
                return True
        elif isinstance(node, ast.ImportFrom):
            if node.module == "framework" or (node.module or "").startswith("framework."):
                return True
    return False


def _imports_lingshu_system(module: ast.Module) -> bool:
    for node in ast.walk(module):
        if isinstance(node, ast.Import):
            if any(alias.name == "lingshu.system" or alias.name.startswith("lingshu.system.") for alias in node.names):
                return True
        elif isinstance(node, ast.ImportFrom):
            if node.module == "lingshu.system" or (node.module or "").startswith("lingshu.system."):
                return True
    return False
...
def _has_forbidden_context_access(module: ast.Module) -> bool:
    forbidden = {"request.app.ctx", "request.app.config", "app.ctx"}
    for node in ast.walk(module):
        if isinstance(node, ast.Attribute):
            path = _attribute_path(node)
            if path in forbidden or any(path.startswith(f"{item}.") for item in forbidden):
                return True
    return False
...
def _check_business_code_errors(root: Path) -> list[str]:
    ...
    for path in files:
        ...
        if module is None:
            continue
        if _imports_legacy_framework(module):
            issues.append(
                f"{_relative(root, path)}: legacy framework import is forbidden; use lingshu imports"
            )
        if _imports_lingshu_system(module):
            issues.append(
                f"{_relative(root, path)}: app code must not import lingshu.system; use from lingshu import config, db, logger, request"
            )
        if _has_forbidden_context_access(module):
            issues.append(
                f"{_relative(root, path)}: direct request.app.ctx/app.ctx access is forbidden; use from lingshu import config, db, logger, request"
            )
