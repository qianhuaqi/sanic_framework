...
def test_error_code_catalog_loads_canonical_locales():
    index = build_error_code_index(Path(__file__).resolve().parents[1] / "language")
    ...
    assert any(item["code"] == "990000" for item in index["codes"])
...
def test_error_code_catalog_uses_version_language_before_project_language(tmp_path):
    module_map = tmp_path / "modules.ini"
    module_map.write_text("[Modules]\n110000-119999 = user\n", encoding="utf-8")

    version_zh_dir = tmp_path / "app" / "v1" / "language" / "zh-CN" / "ERROR"
    project_zh_dir = tmp_path / "language" / "zh-CN" / "ERROR"
    project_en_dir = tmp_path / "language" / "en-US" / "ERROR"
    ...
    index = build_error_code_index(
        [tmp_path / "app" / "v1" / "language", tmp_path / "language"],
...
def test_meta_error_codes_endpoint_is_available_in_testing():
    ...
    assert data["summary"] == {"total": 19, "modules": 5, "reserved": 1}
    ...
    assert any(item["code"] == "990000" for bucket in data["modules"] for item in bucket["codes"])
