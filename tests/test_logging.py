from io import StringIO
from types import SimpleNamespace

from lingshu.system.context import request_context
from lingshu.logging import setup_logging
from lingshu.system import sanic_adapter


def test_setup_logging_isolated_per_app_and_injects_request_metadata(tmp_path):
    log_path = tmp_path / "logs"
    app_a = SimpleNamespace(
        ctx=SimpleNamespace(
            config=SimpleNamespace(
                app_name="test-app-a",
                debug=True,
                log_to_file=False,
                log_level="DEBUG",
                log_path=str(log_path),
                log_file="app.log",
                log_formatter="%(request_id)s|%(method)s|%(path)s|%(user_id)s|%(levelname)s:%(message)s",
                log_max_bytes=2048,
                log_backup_count=2,
            )
        )
    )
    app_b = SimpleNamespace(
        ctx=SimpleNamespace(
            config=SimpleNamespace(
                app_name="test-app-b",
                debug=False,
                log_to_file=False,
                log_level="INFO",
                log_path=str(log_path),
                log_file="app.log",
                log_formatter="%(request_id)s|%(method)s|%(path)s|%(user_id)s|%(levelname)s:%(message)s",
                log_max_bytes=2048,
                log_backup_count=2,
            )
        )
    )

    logger_a = setup_logging(app_a)
    logger_b = setup_logging(app_b)
    assert logger_a is not logger_b
    assert logger_a.name != logger_b.name
    assert logger_a.handlers != logger_b.handlers

    stream = StringIO()
    logger_a.handlers[0].stream = stream

    raw_request = SimpleNamespace(
        app=app_a,
        path="/v1/demo",
        method="POST",
        headers={},
        ctx=SimpleNamespace(g=7, request_id="rid-1"),
    )

    with request_context(app_a, raw_request, request_id="rid-1", user=7):
        logger_a.info("hello")

    for handler in logger_a.handlers:
        handler.flush()

    assert stream.getvalue().strip() == "rid-1|POST|/v1/demo|7|INFO:hello"


def test_setup_logging_writes_to_configured_file(tmp_path):
    log_path = tmp_path / "logs"
    app = SimpleNamespace(
        ctx=SimpleNamespace(
            config=SimpleNamespace(
                app_name="test-app",
                debug=True,
                log_to_file=True,
                log_level="DEBUG",
                log_path=str(log_path),
                log_file="app.log",
                log_formatter="%(levelname)s:%(message)s",
                log_max_bytes=2048,
                log_backup_count=2,
            )
        )
    )

    setup_logging(app)
    logger = sanic_adapter.get_app_logger(app)
    logger.debug("debug message")

    for handler in logger.handlers:
        handler.flush()

    assert (log_path / "app.log").read_text(encoding="utf-8").strip() == "DEBUG:debug message"
