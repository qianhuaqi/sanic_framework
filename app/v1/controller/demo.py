...
async def index(request):
    ...
    logger.debug("demo.index page=%s size=%s use_master=%s", page, size, use_master)
    result = await DemoModel().get_pagination(page=page, size=size, use_master=use_master)
...
async def info(request, data_id):
    ...
    use_master = request.args.get("use_master", "").lower() in {"1", "true", "yes", "on"}
    logger.debug("demo.info id=%s use_cache=%s use_master=%s", data_id, use_cache, use_master)
    item = await DemoModel().get_one(data_id, use_master=use_master, use_cache=use_cache)
    if item is None:
...
async def create(request):
    ...
    data_id = await DemoModel().insert(**payload)
...
async def update(request, data_id):
    require_mysql(request)
    payload = require_payload(request)
    logger.info("demo.update id=%s fields=%s", data_id, sorted(payload.keys()))
    result = await DemoModel().update(data_id, **payload)
...
async def delete(request, data_id):
    require_mysql(request)
    logger.info("demo.delete id=%s", data_id)
    result = await DemoModel().delete(data_id)
